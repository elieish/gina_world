 ...
 */
function dd($var){
	print debug_output($var);
	die();
}

function debug_output($var) {
	$output																= htmlentities(print_r($var, 1));
	$output																= str_replace("\n", "<br>\n", $output);
...
function paginated_listing($query, $this_page="", $prefix="") {
	...
	if ($num_records > $_GLOBALS['max_results']){
		...
		$page_select 													.= "		window.location.replace('$this_page&results_page=' + me.value);\n";
		$page_select 													.= "}\n";
		$page_select 													.= "</script>\n";
...
function generate_select($name, $values, $active="", $use_key=1) {
	# Construct HTML
	$html  																= "<select name='{$name}'>\n";
...
function upload_form($item) {
	...
	<form class='form form-horizontal' enctype='multipart/form-data' action='#' method='POST'>
		...
		<div class='control-group'>
			...
			<div class='controls'> 
				<input name='new_file' type='file' /><br />
			</div>
		</div>
